...
async def _get_user_url(user_name: str) -> str:
    # 构建搜索URL并访问
    search_url = f"https://www.xiaohongshu.com/search_result?keyword={user_name}"
    await main_page.goto(search_url, timeout=60000)
    await asyncio.sleep(5)  # 等待页面加载

    # 点击用户卡片
    user_card = await main_page.query_selector("div#user.channel")
    if user_card:
        await user_card.click()
        await asyncio.sleep(5)  # 等待页面加载
    print("尝试获取用户列表...")
    user_list_items = await main_page.query_selector_all("div.user-list-item")
    print(f"找到 {len(user_list_items)} 个用户")
    _user_url = ""
    for user_list_item in user_list_items:
        user_name_element = await user_list_item.query_selector("div.user-name")
        if user_name_element:
            _user_name = await user_name_element.text_content()
            if _user_name == user_name:
                # 获取链接
                link_element = await user_list_item.query_selector(
                    'a[href*="/user/profile/"]'
                )
                if not link_element:
                    continue
                print(f"找到用户: {user_name}")
                _user_url = await link_element.get_attribute("href")
                print(f"用户URL: {_user_url}")
                break
    return _user_url


async def _get_user_notes(
    user_url: str, user_name: str, contains_top: bool = True, limit: int = 10
) -> str:
    if not user_url:
    ...
    await main_page.goto(full_user_url, timeout=60000)
    await asyncio.sleep(5)  # 等待页面加载
    ...
    for card in post_cards:
        try:
            # 获取链接
            link_element = await card.query_selector('a[href*="/user/profile/"]')
            if not link_element:
                continue

            # 判断置顶
            top_element = await card.query_selector("div.top-wrapper")
            if top_element:
                if contains_top:
                    continue
                else:
    ...
    if unique_posts:
        ...
        return f'未找到与"{user_name}"相关的笔记'


async def _get_note_content(url: str, with_comments: bool = False) -> str:
    # 访问帖子链接
    await main_page.goto(url, timeout=60000)
    await asyncio.sleep(5)  # 增加等待时间到10秒
    # 增强滚动操作以确保所有内容加载
    ...
    logger.error(f'Starting loading page "{url}"...')
    # 打印页面结构片段用于分析
    try:
        print("打印页面结构片段用于分析")
        ...
        print(
            f"页面结构分析: {json.dumps(page_structure, ensure_ascii=False, indent=2)}"
        )
    except Exception as e:
    ...
    logger.error(f'Starting loading title "{url}"...')
    ...
    logger.error(f'Starting loading author "{url}"...')
        ...
        try:
            ...
            for selector in time_selectors:
                ...
                if time_element:
                    time_text = await time_element.text_content()
                    post_content["发布时间"] = (
                        time_text.strip() if time_text else "未知"
                    )
    ...
    logger.error(f'Starting loading time "{url}"...')
    ...
    try:
        ...
        if content_element:
            # 检查是否在评论区域内
            is_in_comment = await content_element.evaluate(
                "(el) => !!el.closest(\"[data-is-comment='true']\") || false"
            )
            if not is_in_comment:
                ...
                if content_text and len(content_text.strip()) > 50:  # 增加长度阈值
                    ...
                    print(
                        f"方法1获取到的内容太短: {len(content_text.strip() if content_text else 0)}"
                    )
    ...
    if post_content["内容"] == "未能获取内容":
        try:
            ...
            if content_text and len(content_text) > 20:
                ...
                print(
                    f"方法2获取到的内容太短或为空: {len(content_text) if content_text else 0}"
                )
        except Exception as e:
    ...
    if post_content["内容"] == "未能获取内容":
        try:
            ...
            if content_text and len(content_text) > 100:  # 增加长度阈值
                ...
                print(
                    f"方法3获取到的内容太短或为空: {len(content_text) if content_text else 0}"
                )
        except Exception as e:
    ...
    if post_content["内容"] == "未能获取内容":
        try:
            ...
            if content_text and len(content_text) > 50:
                ...
                print(
                    f"方法4获取到的内容太短或为空: {len(content_text) if content_text else 0}"
                )
        except Exception as e:
    ...
    if post_content["内容"] == "未能获取内容":
        try:
            ...
            if content_text and len(content_text) > 100:
                ...
                print(
                    f"方法5获取到的内容太短或为空: {len(content_text) if content_text else 0}"
                )
        except Exception as e:
            print(f"方法5获取正文内容出错: {str(e)}")

    logger.error(f'Starting loading content "{url}"...')

    if with_comments:
        post_content["评论"] = await _get_note_comments(main_page)

        logger.error(f'Starting loading comments "{url}"...')
...
async def _get_note_comments(main_page) -> str:
    # 先滚动到评论区
        ...
        if count > 0:
            for i in range(count):
                try:
                    ...
                    # 如果没有找到，尝试通过用户链接查找
                    if username == "未知用户":
                        user_link = comment_element.locator(
                            'a[href*="/user/profile/"]'
                        ).first
                    ...
                    if (
                        ...
                        comments.append(
                            {"用户名": username, "内容": content, "时间": time_location}
                        )
                except Exception:
                    continue
    ...
    if comments:
        ...
        for i, comment in enumerate(comments, 1):
            result += (
                f"{i}. {comment['用户名']}（{comment['时间']}）: {comment['内容']}\n\n"
            )
        return result
    else:
...
async def _search_notes(main_page, keywords: str, limit: int = 5) -> str:
    # 打印页面HTML用于调试
    page_html = await main_page.content()
    print(f"页面HTML片段: {page_html[10000:10500]}...")
    # 使用更精确的选择器获取帖子卡片
...
async def search_notes(
    ...
    try:
        ...
        return await _search_notes(main_page, keywords, limit)
...
async def get_user_notes(
    user_name: str = Field(default="", description="搜索的用户名称"),
    contains_top: bool = Field(default=True, description="是否包含主页置顶笔记"),
    ...
    try:
        ...
        _user_url = await _get_user_url(user_name)

        return await _get_user_notes(_user_url, user_name, contains_top, limit)
...
async def get_note_content(
    url: str = Field(
        default="", description="笔记 URL, 注意请携带全部参数 尤其是 token 类的"
    ),
    with_comments: bool = Field(default=False, description="是否包含评论"),
) -> str:
    """获取笔记内容"""
    login_status = await ensure_browser()
    if not login_status:
    ...
    try:
        return await _get_note_content(url, with_comments)
...
async def get_note_comments(
    ...
    try:
        ...
        return await _get_note_comments(main_page)
...
async def analyze_note(
    ...
    try:
        ...
        # 检查是否获取成功
        if note_content_result.startswith("请先登录") or note_content_result.startswith(
            "获取笔记内容时出错"
        ):
        ...
        words = re.findall(
            r"\w+", f"{post_content.get('标题', '')} {post_content.get('内容', '')}"
        )
        ...
        for domain, domain_keys in domain_keywords.items():
            for key in domain_keys:
                if (
                    key.lower() in post_content.get("标题", "").lower()
                    or key.lower() in post_content.get("内容", "").lower()
                ):
...
@mcp.tool()
async def post_smart_comment(
    url: str = Field(
        default="", description="笔记 URL, 注意请携带全部参数 尤其是 token 类的"
    ),
    comment_type: str = Field(
        default="引流", description="评论类型", choices=["引流", "点赞", "咨询", "专业"]
    ),
) -> dict:
    """
    ...
    Args:
        url: 笔记 URL, 注意请携带全部参数 尤其是 token 类的
...
async def post_comment(
    ...
    comment: str = Field(default="", description="要发布的评论内容"),
) -> str:
    """发布评论到指定笔记"""
    login_status = await ensure_browser()
    if not login_status:
